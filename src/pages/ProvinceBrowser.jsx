...
export default function ProvinceBrowser({ onSelectProvince, mode = 'vehicles', howItWorks }) {
  ...
  const handleClick = async (province) => {
    ...
    if (isMyProvince) {
      ...
      if (userCoords && findNearestProvince(userCoords.latitude, userCoords.longitude)?.name === province.name) {
        onSelectProvince({
          ...province,
      ...
      // Profile has a specific city saved — geocode it for accurate center point
      if (profileCityName) {
        try {
          const coords = await geocodeLocation(`${profileCityName}, ${province.name}, South Africa`);
          if (coords) {
            onSelectProvince({
              ...province,
              lat: coords.latitude,
              ...
              isUserLocation: true,
              locationLabel: profileCityName,
            });
            return;
          }
        } catch { /* fall through to province main city */ }
      }
    }

    // Default: use province main city coords
