...
export default function FindDrivers() {
  ...
  const handleSelectProvince = (province) => {
    ...
    const coords = { latitude: province.lat, longitude: province.lng, displayName: `${province.city}, ${province.name}` };
    autoLocationRef.current = coords;
    setLocationCoords(coords);
    setFilters(prev => ({ ...prev, location: province.city, radius: SEARCH_RADIUS_KM }));
  };
      ...
      <PageHeader
        ...
        subtitle={
          !hasSearched
            ...
            : geocoding
              ? 'Locating…'
              : `${drivers.length} driver${drivers.length !== 1 ? 's' : ''} within ${filters.radius} km of ${selectedProvince?.city || filters.location}`
        }
        backTo="/home"
        action={
          hasSearched && (
